...
public class DragSelectTouchListener implements RecyclerView.OnItemTouchListener {
    ...
    private static final String TAG = "dragSelectListener";
    ...
    public boolean onInterceptTouchEvent(@NonNull RecyclerView rv, @NonNull MotionEvent e) {
        ...
        switch (action) {
            case MotionEvent.ACTION_POINTER_UP, MotionEvent.ACTION_UP -> reset();
        }
    ...
    private void processAutoScroll(MotionEvent event) {
        ...
        if (y >= mTopBoundFrom && y <= mTopBoundTo) {
            ...
            if (mDebug)
                Timber.tag(TAG).d("SCROLL - mScrollSpeedFactor=" + mScrollSpeedFactor + " | mScrollDistance=" + mScrollDistance);
            if (!mInTopSpot) {
                mInTopSpot = true;
                startAutoScroll();
            }
    ...
    private void scrollBy(int distance) {
        int scrollDistance;
        if (distance > 0) scrollDistance = Math.min(distance, mMaxScrollDistance);
        else scrollDistance = Math.max(distance, -mMaxScrollDistance);
