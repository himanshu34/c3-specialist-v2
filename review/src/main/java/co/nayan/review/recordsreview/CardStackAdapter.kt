...
import co.nayan.review.databinding.RecordCardViewRowBinding
import co.nayan.review.recordsgallery.RecordItem
import co.nayan.review.utils.GifRequestListener
...
class CardStackAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CardStackHolder {
        return CardStackHolder(
            RecordCardViewRowBinding.inflate(
    ...
    fun addAll(toAdd: List<RecordItem>): Int {
        val newRecords = toAdd.filter { records.contains(it).not() }
        records.addAll(newRecords)
        return newRecords.size
    }

    fun getItem(position: Int): RecordItem? {
    ...
    inner class CardStackHolder(
        val binding: RecordCardViewRowBinding,
        private val cardStackListener: CardRecordListener
    ) : RecyclerView.ViewHolder(binding.root) {

//        private val recordIdTxt = itemView.recordIdTxt
//        private val overlayTransparentIv = itemView.overlayTransparentIv
...
//        private var needsNoRejectionView = itemView.needsNoRejectionView
        ...
        private val gifRequestListener = object : GifRequestListener() {
            ...
            override fun onSuccess(resource: GifDrawable?) {
                onBitmapLoadSuccess()
            }
        }
            ...
            if (annotations.isNullOrEmpty()) {
                ...
                val filteredAnnotations = if (toggledTemplate == null) annotations
                    ...
                    when (record?.applicationMode) {
                        Mode.MCML -> annotations.filter { it.input == toggledTemplate?.templateName }
                        Mode.MCMT -> annotations.filter { it.tags?.contains(toggledTemplate?.templateName) == true }
                        else -> annotations
                    }
                }
