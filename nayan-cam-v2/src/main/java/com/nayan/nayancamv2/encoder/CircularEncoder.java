...
public class CircularEncoder {
    ...
    public interface Callback {
         ...
         */
        void fileSaveComplete(int status, RecordingData recordingData);
     ...
     */
    public CircularEncoder(int width, int height, int bitRate, int frameRate, int desiredSpanSec, int orientationHint, Callback cb) throws Exception {
        ...
        // room for at least one full GOP in the buffer, preferably two.
        ...
        mEncoder = createVideoEncoder(width, height, bitRate, frameRate, orientationHint);
        ...
        mEncoderThread.setOrientationHint(orientationHint);
        mEncoderThread.start();
        mEncoderThread.waitUntilReady();
    }

    private MediaCodec createVideoEncoder(int mVideoWidth, int mVideoHeight, int bitRate, int frameRate, int orientationHint) throws Exception {
        ...
        if(!GlobalParams.INSTANCE.isCameraExternal()) videoFormat.setInteger(MediaFormat.KEY_ROTATION, orientationHint);
        Timber.tag(TAG).d("format: %s", videoFormat);
    ...
    public void shutdown() {
        Timber.tag(TAG).d("releasing encoder objects");
        ...
        try {
            ...
            Timber.tag(TAG).e("Encoder thread join() was interrupted " + ie);
        }
     ...
     */
    public void saveVideo(RecordingData recordingData) {
        Handler handler = mEncoderThread.getHandler();
        handler.sendMessage(handler.obtainMessage(EncoderThread.EncoderHandler.MSG_SAVE_VIDEO, recordingData));
    }
    ...
    private static class EncoderThread extends Thread {
        ...
        public void run() {
            ...
            Timber.tag(TAG).d("encoder thread ready");
            synchronized (mLock) {
                mReady = true;
            ...
            Timber.tag(TAG).d("FVlooper quit");
        }
            ...
            try {
                ...
                while (true) {
                    ...
                    if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                        ...
                        Timber.tag(TAG).d("encoder output format changed: %s", mEncodedFormat);
                    } else if (encoderStatus < 0) {
                        Timber.tag(TAG).w("unexpected result from encoder.dequeueOutputBuffer: %s", encoderStatus);
                        // let's ignore it
                    } else {
        ...
        void frameAvailableSoon() {
            //if (VERBOSE) Timber.tag(TAG).d("frameAvailableSoon");
            drainEncoder();

            mFrameNum++;
            if ((mFrameNum % 10) == 0) {
                // should base off frame rate or clock?
         ...
         */
        void saveVideo(RecordingData recordingData) {
            File outputFile = recordingData.getFile();
            Timber.tag(TAG).d("*********************Save Video called *******************");
            ExecutorService executorThread = Executors.newSingleThreadExecutor();
            executorThread.execute(() -> {
                if (VERBOSE) Timber.tag(TAG).d("saveVideo %s", outputFile);
                int index = mEncBuffer.getFirstIndex();
                if (index < 0) {
                    Timber.tag(TAG).w("Unable to get first index");
                    mCallback.fileSaveComplete(1, recordingData);
                    return;
                }
                ...
                int result;
                try {
                    Timber.tag(TAG).d("🦀filesavepath: %s", outputFile.getPath());
                    muxer = new MediaMuxer(outputFile.getPath(), MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
                    if (!GlobalParams.INSTANCE.isCameraExternal())
                    ...
                    do {
                        ...
                        Timber.tag(TAG).d("SAVE " + index + " flags=0x" + Integer.toHexString(info.flags));
                        muxer.writeSampleData(videoTrack, buf, info);
                        index = mEncBuffer.getNextIndex(index);
                        Timber.tag(TAG).d("next index -->>> %s", index);
                        Timber.tag(TAG).d("************************END********************************\n");
                    } while (index >= 0);
                    result = 0;
                } catch (Exception ex) {
                    FirebaseCrashlytics.getInstance().recordException(ex);
                    Timber.tag(TAG).e(ex, "muxer failed");
                    result = 2;
                } finally {
                    try {
                        //adding try catch here since this needs to be executed but may cause exception if system shuts down muxer pre-emptively
                        if (muxer != null) {
                            muxer.stop();
                            muxer.release();
                        }
                ...
                if (VERBOSE) Timber.tag(TAG).d("muxer stopped, result=%s", result);
                mCallback.fileSaveComplete(result, recordingData);
            });
        ...
        private static class EncoderHandler extends Handler {
            ...
            public void handleMessage(Message msg) {
                int what = msg.what;
                //if (VERBOSE) Timber.tag(TAG).i("EncoderHandler: what=%s", what);

                EncoderThread encoderThread = mWeakEncoderThread.get();
                if (encoderThread == null) {
                    Timber.tag(TAG).i("EncoderHandler.handleMessage: weak ref is null");
                    return;
                }

                switch (what) {
                    case MSG_FRAME_AVAILABLE_SOON -> encoderThread.frameAvailableSoon();
                    case MSG_SAVE_VIDEO -> encoderThread.saveVideo((RecordingData) msg.obj);
                    case MSG_SHUTDOWN -> encoderThread.shutdown();
