...
class AttendanceSyncWorker(
    ...
    override suspend fun doWork(): Result = coroutineScope {
        return@coroutineScope try {
            DaggerNayanCamComponent.builder()
                ...
                ).build().inject(this@AttendanceSyncWorker)
            ...
            if (nayanCamModuleInteractor.isLoggedIn() && currentRoles.contains(Role.DRIVER)) {
                ...
                async { attendanceSyncManager.pushDriverAttendanceData(isUserLoggingOut) }.await()
                Result.success()
            } else Result.failure()
        } catch (e: Exception) {
    ...
    override suspend fun getForegroundInfo(): ForegroundInfo {
        val title = appContext.getString(R.string.database_sync_started)
        val message = appContext.getString(R.string.uploading_assets)
    ...
    private fun getUploadNotification(title: String, message: String): Notification {
        val channelId = "com.nayan.database_sync_service"
