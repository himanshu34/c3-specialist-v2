...
class YuvToRgbConverter(context: Context) {
    ...
    fun yuvToRgb(image: Image): Bitmap? {
        ...
        if (::yuvBuffer.isInitialized.not()) {
        ...
        if (::inputAllocation.isInitialized.not()) {
        ...
        if (::outputAllocation.isInitialized.not())
            outputAllocation = Allocation.createFromBitmap(rs, bitmapBuffer)

        // Convert NV21 format YUV to RGB
        inputAllocation.copyFrom(yuvBuffer)
        scriptYuvToRgb.setInput(inputAllocation)
        scriptYuvToRgb.forEach(outputAllocation)
        outputAllocation.copyTo(bitmapBuffer)
        return bitmapBuffer
    }
    ...
    private fun imageToByteArray(image: Image, outputBuffer: ByteArray) {
        ...
        imagePlanes.forEachIndexed { planeIndex, plane ->
            ...
            val planeCrop = if (planeIndex == 0) imageCrop
            else {
                Rect(
                    imageCrop.left / 2,
                    imageCrop.top / 2,
