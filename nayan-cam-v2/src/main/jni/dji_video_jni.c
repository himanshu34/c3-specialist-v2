 ...
 * Codec information
 */
JNIEXPORT jstring Java_com_nayan_nayancamv2_extcam_media_NativeHelper_codecinfotest(JNIEnv *env, jobject obj)
{
    char info[40000] = { 0 };
...
{
    // Directly create a Java byte array from the buffer without copying
    jbyteArray jarray = (*env)->NewByteArray(env, size);
    (*env)->SetByteArrayRegion(env, jarray, 0, size, (jbyte*)buf);

    // Call the Java method with the converted data
 ...
 */
JNIEXPORT jboolean Java_com_nayan_nayancamv2_extcam_media_NativeHelper_init(JNIEnv *env, jobject obj)
{
    jclass clazz = (*env)->GetObjectClass(env, obj);
    dataCallbackMID = (*env)->GetMethodID(env, clazz, "onFrameDataReceived", "([BIIZII)V");
...
int parse(JNIEnv *env, jobject obj, uint8_t* pBuff, int videosize, uint64_t pts)
{
    ...
    while (paserLength_In > 0)
    {
        ...
        if (m_pCodecPaser == NULL) {
            ...
            Java_com_nayan_nayancamv2_extcam_media_NativeHelper_init(env, obj);
        }
        if (m_pCodecCtx == NULL) {
 ...
 */
JNIEXPORT jboolean Java_com_nayan_nayancamv2_extcam_media_NativeHelper_parse(JNIEnv *env, jobject obj, jbyteArray pBuff, int size)
{
 ...
 * Release the ffmpeg.
 */
JNIEXPORT jboolean Java_com_nayan_nayancamv2_extcam_media_NativeHelper_release(JNIEnv *env, jobject obj)
{
    if (m_pCodecCtx)
    {
