...
@AndroidEntryPoint
class IncorrectJudgmentsDetailActivity : BaseActivity() {
    ...
    private var incorrectJudgment: IncorrectJudgment? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = getString(R.string.incorrect_judgment)
        binding.makeTransparent.setOnTouchListener(makeTransparentListener)
        binding.yourAnswerHeadTxt.text = getString(R.string.your_answer)
        binding.correctAnswerHeadTxt.text = getString(R.string.correct_answer)
    ...
    private val videoViewClickListener = View.OnClickListener {
        incorrectJudgment?.let { judgment ->
            Intent(
                this@IncorrectJudgmentsDetailActivity,
                VideoTypeRecordActivity::class.java
            ).apply {
                putExtra(Extras.RECORD, judgment.dataRecord?.createDataRecord(judgment.annotation))
                putExtra(
                    Extras.QUESTION,
                    judgment.wfStep?.question.question(judgment.annotation?.answer())
                )
                startActivity(this)
            }
        }
    ...
    private fun setupExtras() {
        incorrectJudgment = intent.parcelable(Extras.INCORRECT_JUDGMENT)
        if (incorrectJudgment == null) showMessage(getString(co.nayan.c3v2.core.R.string.something_went_wrong))
        else incorrectRecordDetailsViewModel.fetchRecord(incorrectJudgment?.dataRecord?.id)
    }

    private fun setupRecord(toSet: Record?) = lifecycleScope.launch {
        incorrectJudgment?.dataRecord = toSet
        incorrectJudgment?.let { judgment ->
            try {
                binding.recordIdTxt.text = "${judgment.dataRecord?.id}"
                binding.questionTxt.text =
                    String.format(judgment.wfStep?.question.question(judgment.annotation?.answer()))

                if (judgment.dataRecord?.mediaType == MediaType.VIDEO) {
                    binding.videoViewContainer.visible()
                    binding.videoView.gone()
                } else {
                    val url = if (judgment.dataRecord?.mediaType == MediaType.VIDEO)
                        judgment.dataRecord?.mediaUrl
                    else judgment.dataRecord?.displayImage ?: judgment.dataRecord?.mediaUrl
                    url?.let { mediaUrl ->
                        if (mediaUrl.isVideo()) {
                            ...
                            binding.videoView.gone()
                            drawType = judgment.annotation?.drawType()
                            drawType.view().visible()
                            drawType.view().setImageBitmap(null)

                            val imageUrl = judgment.dataRecord?.displayImage
                            if (imageUrl?.contains("gif") == true) {
                                ...
                                originalBitmap =
                                    getOriginalBitmapFromUrl(judgment.dataRecord?.displayImage)
                            }
                            setupAnnotation(judgment.annotation)
                        }
                    }
                }

                val userJudgment = judgment.judgment ?: false
                binding.correctAnswer.text = "${!userJudgment}"
                binding.yourAnswer.text = "$userJudgment"
    ...
    private fun isBitmapInitialized() =
        this@IncorrectJudgmentsDetailActivity::originalBitmap.isInitialized
