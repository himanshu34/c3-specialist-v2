...
import co.nayan.c3specialist_v2.databinding.ActivityManagerWorkSummaryBinding
...
@AndroidEntryPoint
class ManagerWorkSummaryActivity : BaseActivity() {

    private val managerWorkSummaryViewModel: ManagerWorkSummaryViewModel by viewModels()
    private val binding: ActivityManagerWorkSummaryBinding by viewBinding(
        ActivityManagerWorkSummaryBinding::inflate
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupEndDate()
        managerWorkSummaryViewModel.state.observe(this, stateObserver)
        managerWorkSummaryViewModel.stats.observe(this, userStatsObserver)
        setupClicks()
    ...
    private val userStatsObserver: Observer<StatsResponse?> = Observer {
        val stats = it?.stats
        binding.potentialPointsTxt.text = stats?.potentialScore ?: "00:00"
        ...
        binding.totalReviewsCountTxt.text = (stats?.totalReviews ?: 0).toString()
        binding.completedReviewsCountTxt.text = (stats?.completedReviews ?: 0).toString()
    ...
    private fun setupClicks() {
        ...
        binding.dateRangeLayout.filterBtn.setOnClickListener {
            managerWorkSummaryViewModel.fetchWorkSummary(startDate(), endDate())
        }
    }
